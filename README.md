...
//
Por alguna razón el el README del PI Food NO están las instrucciones para crear el repositorio en GitHub, por eso hago un copy-paste de esas instrucciones del PI de Videogames (FT14a) por si alguien las necesita:
 1. Se debe iniciar git con `git init`  (Usando la consola y estando dentro de la carpeta del PI)
 2. Crear en su cuenta de GitHub un nuevo repo con el nombre `PI-Food-FT__` donde `__` es el numero de cohorte en el que estas    (cambiar la palabra Food si tu PI es otro diferente)
 3. Hacer un commit inicial ej: `git add .` => `git commit -m "init"` => `git branch -M main`
 4. Seguir las instrucciones de GitHub para 'conectar' su repositorio local con GitHub `git remote add........`       (los puntos suspensivos aparecerán en GitHub como la dirección de tu repositorio recién creado)
